#!/usr/bin/env python
"""
Weaver Architecture Generator — reads the codebase and produces:
...
Adapted from flux/docs/architecture/generate.py.
"""
...
def gen_dataflow_mermaid(plugins):
    ...
    events_map = {
        "commit-intelligence": ["weaver.commit.drafted", "weaver.commit.committed"],
        "boundary-segmenter": ["weaver.task.boundary.detected"],
        "branch-workflow": ["weaver.workflow.detected", "weaver.branch.created"],
        "pr-lifecycle": ["weaver.pr.drafted", "weaver.pr.ready", "weaver.pr.merged"],
        "weaver-gate": ["weaver.destructive.detected", "weaver.destructive.confirmed"],
        "capability-memory": ["capability_loaded", "gitlab_version_probed"],
        "ci-reader": ["weaver.ci.status.observed"],
        "weaver-learning": ["priors_loaded", "preferences_checkpointed"],
    }
...
<head>
<meta charset="utf-8">
<title>Weaver — Architecture Explorer</title>
...
<div class="container">
  <h1>Weaver <span>Architecture Explorer</span></h1>
