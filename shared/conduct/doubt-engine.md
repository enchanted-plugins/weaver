...
3. You're about to declare a verdict (READY, MERGEABLE, DONE, "looks good") on your own output.
...
|---|---|---|
| "Just force-push, it's my branch" | "OK, force-pushing." | "`weaver-gate` flags this — the branch has 2 commits the remote doesn't, and `git reflog` shows you co-authored one of them yesterday on a different machine. Force-push will lose that commit. Want me to fetch + inspect first?" |
| "Squash everything into one commit before opening the PR" | "Sure, squashing." | "W2 segmented this into 3 distinct boundaries (refactor, feature, test fixup). Squashing collapses them into one Conventional Commits subject — the Haiku validator will reject. Stack the commits or amend the boundaries first?" |
| *(your own prior claim)* "the workflow is trunk-based" | "Right, trunk-based — no branch needed." | "I framed that too narrowly — `state/capability-registry.json` shows protected `release/*` branches and a 14-day tag cadence; W3 reclassifies as release-flow. Branch needed after all." |
...
- **Skipping the pass under "just do it" license.** "Just do it" overrides clarifying questions, not safety pushback. If the action is destructive or known-failed, the pass still fires — `weaver-gate` exists for exactly this reason.
